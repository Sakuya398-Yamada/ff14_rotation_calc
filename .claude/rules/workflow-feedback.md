...
1. #195 の全コメントを読み、末尾に ✅ マーカーが無い **重要度 High / Medium** の知見を独立Issueへ昇格起票する（ラベルは内容に応じて `feature` / `refactor` / `docs` 等を付与）
...
- ルーティンの昇格対象外。マーカーが付かないまま残る
- ユーザーまたは手動セッションで随時昇格してよい。昇格した場合は同じ ✅ マーカーを必ず付ける
