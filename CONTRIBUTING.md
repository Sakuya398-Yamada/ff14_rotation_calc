...
Claude Codeで `/issue-start #<番号>` を実行すると、以下を自動で行います。

1. Issueの内容（要件・設計・定義）を読み取る
2. 規約に沿ったブランチを作成する
3. Issueの要件に基づいて実装を進める

### 4. PR作成・マージ

実装完了後、PRを作成します。PRの本文に `closes #<issue番号>` を含めることで、マージ時にIssueが自動クローズされます。
