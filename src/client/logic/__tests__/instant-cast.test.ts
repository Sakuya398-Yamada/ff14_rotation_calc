...
describe("複数 instantCast バフの優先順位（単発バフ優先）", () => {
  // 迅速魔（単発10s）と三連魔（スタック式 maxStacks:3, 15s）が同時アクティブの状態で
  // 詠唱 GCD を実行すると、両方のバフが消費されてしまう不具合 (#202) の回帰テスト。
  // 仕様: 単発バフ（maxStacks 未定義）を優先して 1 発につき 1 つだけ消費する。
  const swiftcastBuff: BuffDefinition = {
    id: "swiftcast-test",
    name: "迅速魔",
    shortName: "迅速魔",
    icon: "",
  ...
  it("迅速魔（単発）と三連魔（スタック式）が同時有効な詠唱GCDで、迅速魔のみ消費し三連魔は据え置く", () => {
    const grantSwift = makeSkill({ id: "grant-swift", buffApplications: ["swiftcast-test"] });
    const grantTriple = makeSkill({ id: "grant-triple", buffApplications: ["triplecast-test"] });
    const castSpell = makeSkill({ id: "cast-spell", castTime: 2.8 });
    const skillMap = new Map([
      [grantSwift.id, grantSwift],
      [grantTriple.id, grantTriple],
      [castSpell.id, castSpell],
    ]);

    const result = resolveTimeline(
      [
        makeEntry("grant-triple"),
        makeEntry("grant-swift"),
        makeEntry("cast-spell"),
        makeEntry("cast-spell"),
        makeEntry("cast-spell"),
        makeEntry("cast-spell"),
      ...
      [],
      undefined,
      [swiftcastBuff, triplecastBuff]
    );

    // 1発目の cast-spell は両バフが効いている状態で詠唱破棄、迅速魔のみ消費される。
    expect(result.entries[2].castTime).toBe(0);
    // 1発目消費直後：迅速魔は消失、三連魔はまだ存在（スタック据え置き）
    expect(
      result.entries[2].activeBuffs.some((ab) => ab.buffId === "swiftcast-test")
    ).toBe(false);
    expect(
      result.entries[2].activeBuffs.some((ab) => ab.buffId === "triplecast-test")
    ).toBe(true);

    // 2〜4発目は三連魔だけで詠唱破棄を続ける（3 スタックを順に消費）。
    expect(result.entries[3].castTime).toBe(0);
    expect(result.entries[4].castTime).toBe(0);
    expect(result.entries[5].castTime).toBe(0);

    // 4発目時点では三連魔も消費しきって失効している（合計 3 発の詠唱破棄を担当）。
    expect(
      result.entries[5].activeBuffs.some((ab) => ab.buffId === "triplecast-test")
    ).toBe(false);
  });

  it("単発バフ同士（ファイアスターター + 迅速魔）が同時有効な場合は配列順で 1 つだけ消費", () => {
    const grantFs = makeSkill({ id: "grant-fs", buffApplications: ["firestarter-test"] });
    const grantSwift = makeSkill({ id: "grant-swift", buffApplications: ["swiftcast-test"] });
    const castSpell = makeSkill({ id: "cast-spell", castTime: 2.8 });
    const skillMap = new Map([
      [grantFs.id, grantFs],
    ...
    const result = resolveTimeline(
      ...
      [],
      undefined,
      [firestarterBuff, swiftcastBuff]
    );

    // 1〜2発目は単発バフが順番に消費されて Instant 化、3発目は両方失効済みで通常詠唱。
    expect(result.entries[2].castTime).toBe(0);
    expect(result.entries[3].castTime).toBe(0);
    expect(result.entries[4].castTime).toBeCloseTo(2.8, 3);
    ...
    // 2発目消費後は両バフとも消失している。
    expect(
      result.entries[3].activeBuffs.some(
        (ab) => ab.buffId === "firestarter-test" || ab.buffId === "swiftcast-test"
      )
    ).toBe(false);
  });

  it("三連魔のみ（単発バフなし）の場合は従来どおりスタックを 1 ずつ消費する（回帰チェック）", () => {
    const grantTriple = makeSkill({ id: "grant-triple", buffApplications: ["triplecast-test"] });
    const castSpell = makeSkill({ id: "cast-spell", castTime: 2.8 });
    const skillMap = new Map([
      [grantTriple.id, grantTriple],
    ...
    expect(result.entries[4].castTime).toBeCloseTo(2.8, 3);
    expect(
      result.entries[4].activeBuffs.some((ab) => ab.buffId === "triplecast-test")
    ).toBe(false);
  });
});
