...
1. **Issue作成**: `/issue-create` で構造化されたIssueを起票
2. **開発計画**: `/dev-plan` で依存関係を分析し、Wave構造に分解
3. **開発開始**: `/issue-start #<番号>` でブランチ作成＆開発開始
4. **実装**: Issue内容に基づき実装
5. **PR作成**: `closes #<issue番号>` を含めたPRを作成
6. **レビュー＆マージ**: PRマージでIssueが自動クローズ
