...
- **応答言語**: ユーザーへのチャット応答・報告は常に日本語で書く（コード・コミットメッセージ等は既存規約に従う）
...
- **コード読みは必要最小限**: 全文理解目的の一括読みは禁止。Grep で該当行を特定してから `Read` に `offset`/`limit` を付けて局所読みする。800 行超のファイルは全読みしない（均質な低密度ファイルの例外・高密度ロジックの注記あり）。3ファイル以上の横断読みは `code-explorer` サブエージェントに委譲し、プロンプトに返却量の上限（例: 「`file:line` と 5 行以内の要約のみ」）を必ず明記する。詳細は `@.claude/rules/context-efficiency.md`
